...
<p><?php echo $message; ?></p>
    ...
    <ul class="course">
        ...
        $q_courses = "SELECT DISTINCT course_name FROM bca WHERE semester = 'S2'";
        ...
        while ($course_row = mysqli_fetch_assoc($p_courses)) {
            ...
            $q_pdfs = "SELECT * FROM bca WHERE course_name = '$courseName' AND semester = 'S2'";
   ...
   function performSearch(query, type, resultsContainer) {
    if (query.trim() !== "") {
        ...
        fetch("search_bca_s2.php?q=" + query + "&type=" + type)
            .then(response => {
                if (!response.ok) {
