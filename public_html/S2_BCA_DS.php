...
<p><?php echo $message; ?></p>
    ...
    <ul class="course">
        ...
        // Fetch all distinct course names for S2 BCA
        $q_courses = "SELECT DISTINCT course_name FROM bca_ds WHERE semester = 'S2'";
        ...
        while ($course_row = mysqli_fetch_assoc($p_courses)) {
            ...
            $q_pdfs = "SELECT * FROM bca_ds WHERE course_name = '$courseName' AND semester = 'S2'";
   ...
   function performSearch(query, type, resultsContainer) {
    if (query.trim() !== "") {
        ...
        fetch("search_bca_ds_s2.php?q=" + query + "&type=" + type)
            .then(response => {
                if (!response.ok) {
