...
.delete-btn:hover {
    ...
    if (confirmation) {
        // Use AJAX to delete the record without navigating
        ...
        // Update the URL to the correct file handling the deletion logic
        xmlhttp.open("GET", "path/to/your/delete_file.php?delete_id=" + id, true);
        xmlhttp.send();
    } else {
    ...
    function deleteAllRecords() {
        ...
        if (confirmed) {
            // Use AJAX to delete all records without navigating
            var xmlhttp = new XMLHttpRequest();
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
            ...
            xmlhttp.open("GET", "cloudandnfc.000webhostapp.com/admin_login.php?delete_all=1", true);
            xmlhttp.send();
        }
    }
</script>
...
<script>
    function deleteAllRecords() {
        var confirmed = confirm('Are you sure you want to delete all records?');
        if (confirmed) {
            echo "<td><a href='javascript:void(0)' onclick='deleteRequest({$row['id']})'>Delete</a></td>";
        }
    }
</script>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["update_message"])) {
    $message_id = 1; // Change this to the desired message ID
    $new_message = $_POST["new_message"];

    // SQL query to update the message with the specified ID
    $sql = "UPDATE message SET message = '$new_message' WHERE id = $message_id";
