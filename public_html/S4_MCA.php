...
<p><?php echo $message; ?></p>
    ...
    <ul class="course">
        ...
        // Fetch all distinct course names for S4 MCA
        $q_courses = "SELECT DISTINCT course_name FROM mca WHERE semester = 'S4'";
        ...
        while ($course_row = mysqli_fetch_assoc($p_courses)) {
            ...
            $q_pdfs = "SELECT * FROM mca WHERE course_name = '$courseName' AND semester = 'S4'";
   ...
   function performSearch(query, type, resultsContainer) {
    if (query.trim() !== "") {
        ...
        fetch("search_mca_s4.php?q=" + query + "&type=" + type)
            .then(response => {
                if (!response.ok) {
